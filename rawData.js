...
// Dev info.
// 144 objects in victims

function propChecker() {
    const victimClone = [...victims];
    const props = ['name', 'birthday', 'died', 'location', 'description'];
    victimClone.forEach((obj, index) => {
        if(!props.every(item => obj.hasOwnProperty(item))) {
...
function locationFixer() {
    const victimClone = [...victims];
        victimClone.forEach(obj => {
            let splitter = obj.location.split(',');
            if (splitter.length > 2) {
                obj["location"] = {
                    neighborhood: splitter[0].trim(), 
                    city: splitter[1].trim(),
                    state: splitter[2].trim(),
                }
            } else {
                obj["location"] = {
                    neighborhood: null, 
                    city: splitter[0].trim(),
                    state: splitter[1].trim(),
                }
            }
        })
    console.dir(victimClone, {'maxArrayLength': 150});
};